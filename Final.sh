...
install_kalitorify() {
    ...
    if command -v kalitorify &> /dev/null; then
        log "✅ Kalitorify instalado correctamente."
    else
        log "❌ Kalitorify no se encuentra en el PATH."
    fi
}
...
main_menu() {
  while true; do
    ...
    echo "2) 📊 Instalar herramientas de monitoreo"
    echo "3) 🔍 Instalar Kalitorify"
    echo "4) ❌ Eliminar Kalitorify"
    echo "5) ♻ Revertir TODO"
    echo "6) Salir"
    ...
    case $option in
      ...
      3) install_kalitorify ;;
      4) remove_kalitorify ;;
      5) revert_all ;;
      6) log "Saliendo..."; exit 0 ;;
